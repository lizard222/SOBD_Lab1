# SOBD_Lab1## Лабораторная работа № 1
...
**Маттиев Рустам, САПР-1.1 ВолгГТУ**
---
...
4. Скачайте датасет, положите его в директорию `data` проекта (если директория отсутствует, создайте ее),а затем исполльзуйте следующую команду, чтобы загрузить датасет в HSFS:
```bash
hdfs dfs -copyFromLocal /work/data/<Название файла> data
```
5. Запустите докер-контейнер со средой разработки и инструментом `Apache Spark` в репозитории и дождитесь завершения его работы.
> `$ docker-compose up`
6. Откройте браузер и перейдите по адресу: http://localhost:10000/lab
7. Выполните разведочный анализ выбранного датасета с определением: 
...
9. Напишите **главу курсовой работы** в пределах **10 страниц**, в которой опишите постановку задачи, описание датасета со ссылкой на него, проведенный разведовательный анализ и выводы.
10. Сохраните ноутбук с проведенным анализом и написанную главу курсовой в `GitHub / GitLab`.
